			...
			<view class="uni-list-cell" hover-class="uni-list-cell-hover" v-for="(item,index) in devhistory" :key="index">
				...
				<view class="uni-media-list" v-if="item.data.infostring">
					<view class="uni-media-list-body">
						<view class="uni-media-list-text-top">温度:&nbsp;{{item.data.infostring.T}}'C&nbsp;湿度:&nbsp;{{item.data.infostring.H}}%&nbsp;光强:&nbsp;{{item.data.infostring.L}}cd</view>
						<view class="uni-media-list-text-bottom uni-ellipsis">时间：{{item.timestamp.substring(0,4)}}.{{item.timestamp.substring(4,6)}}.{{item.timestamp.substring(6,8)}} {{parseInt(item.timestamp.substring(9,11))+8}}:{{item.timestamp.substring(11,13)}}:{{item.timestamp.substring(13,15)}}</view>
					</view>
				</view>
			</view>
		</view>
